...
export default function Footer() {
  ...
  const buttons = data.map((item) => <li className={activeClass(item.id, filter)} key={item.text.length}><button type="button" onClick={() => changeActive(item.id)}>{item.text}</button></li>);
